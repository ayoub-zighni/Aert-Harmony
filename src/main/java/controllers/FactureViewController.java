...
public class FactureViewController {
    ...
    private void afficherFactures() {
        try {
            PreparedStatement statement = connection.prepareStatement("SELECT * FROM facture WHERE id_userfact = ?");
            statement.setInt(1, utilisateurconnect.getId()); // Bind the user ID to the prepared statement
